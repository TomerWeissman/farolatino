...
1. **Load config**: Call `load_config("search_criteria")` and `get_profile(profile_name)` (or "default" if none specified). Merge any filter_overrides from the profile into the base search criteria.

2. **Discovery queries**: Call `chartmetric_discovery` with the merged filters. This runs:
...
4. **Score all candidates**: For each candidate (up to 50):
   a. Check `cache_get` — skip full pull if recently scored and data is fresh
   b. Call `chartmetric_artist` for full data
   c. Call `compute_prospect_score` with the active profile
   d. Call `cache_set` to store results

5. **Rank**: Sort all scored candidates by prospect_score descending. Take the top 20.

6. **Enrich top prospects**: For the top 20, call `estimate_revenue` and `generate_dossier`.

7. **Alert routing**: Call `route_alert` for each top prospect. Collect all HOT and WARM alerts, plus any signal alerts.
...
## Notes
- Available profiles: default, emerging_momentum, revenue_focus, latam_expansion (or run `list_profiles` to see all)
