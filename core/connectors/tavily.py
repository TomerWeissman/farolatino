...
def web_search(query: str, max_results: int = 5) -> dict:
    ...
    if resp.status_code in (401, 403):
        return {"error": "Tavily auth failed — check TAVILY_API_KEY in Connections."}
    if resp.status_code == 429:
        return {"error": "Tavily quota exhausted — wait or upgrade your plan."}
    if resp.status_code >= 400:
        return {"error": f"Tavily HTTP {resp.status_code}: {resp.text[:200]}"}
